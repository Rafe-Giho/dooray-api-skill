...
if (args.help) {
  console.log('Usage: node dooray-api-check.mjs [--project AI기술혁신부(SE2)] [--wiki AI기술혁신부(SE2)] [--json]');
  process.exit(0);
}
const loaded = loadConfig(args.config);
const { config } = loaded;
const results = [];
results.push(await check('config/token', async () => {
...
if (args.json) console.log(JSON.stringify(out, null, 2));
  ...
  for (const r of results) console.log(`- ${r.name}: ${r.ok ? 'ok' : `failed (${r.error})`}`);
}
process.exit(ok ? 0 : 1);
